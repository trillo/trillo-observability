...
| `TAO-Plugin-Internal-Setup.md` | Internal team guide: OAuth client registration (OAuthClientRegistry), pointing the plugin at dev via TAO_MCP_URL, and invalid_client/auth troubleshooting. **Internal.** |
| `TAO-Plugin-UI-Login-Handoff.md` | UI-team spec: the login-first OAuth completion handshake an AOS app SPA must implement (detect oauth_state !92 authenticate !92 POST /oauth/complete !92 loopback redirect). **Internal.** |
