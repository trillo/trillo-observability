...
| Trillo execution correlation | `execution_id=` kwarg | Both sinks get `attrs.trillo.execution_id`. Postgres additionally populates a top-level `executionId` column (first-class on the Postgres side, unchanged). **On OliverDB, `executionId` is now also a first-class column — its purpose is to avoid `traceId` collisions across services / producers within a tenant and to handle non-OTel-compliant `traceId`s emitted by customer agents.** The ingest plugin promotes `attrs.trillo.execution_id` into the `executionId` column for Trillo-emitted rows; for customer telemetry (ADK / LangChain / LangGraph / CrewAI) it derives `executionId = UUIDv5(trillo_ns, service.namespace ‖ traceId)` — deterministic UUID, same for every span in a trace regardless of arrival order, disambiguated across producers via `service.namespace`. Consumers filter by `executionId` on both sinks; no consumer code needs to know the origin of the row. |
