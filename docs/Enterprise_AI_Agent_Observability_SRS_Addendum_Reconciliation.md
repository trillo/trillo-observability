...
- **Topology from spans → `agent_dependency` edges**, `discovery_source`
...
- **Analytical / background tables** — `metric_rollup`, `platform_finding`,
  `analysis_baseline`, `sweeper_run`, `ai_analysis` (PRD §11.4). Needed for the
...
- **Governance detail** — `governance_policy_version` table, `governance_decision`,
...
| :- | :- | :- | :- |
| Logical agent table | `agent` | `agent_inventory` | (decide) |
| Governance outcome | `governance_evaluation` | `governance_decision` | `governance_decision` (clearer) |
| Admin audit | `administrative_audit` | `administrative_audit_event` | (align) |
